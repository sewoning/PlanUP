...
## Adaptation notes for PlanUP (Korean, dense dashboard UI)

This app is a dense daily-use dashboard (calendar grid, kanban cards, data tables, budget panels), not a spacious marketing site, and all copy is Korean (Pretendard), not Latin Inter. Applied literally, several of the marketing-site numbers above would break the dense layouts or look wrong on Hangul. Adaptations made when applying this system to `index.html`:

- **Font**: kept Pretendard (already the Korean-tuned equivalent role that `NotionInter` plays for Latin) instead of swapping to Inter.
- **Letter-spacing**: all of Notion's negative tracking values are Latin-only tricks (Inter reads loose at display sizes without them). Hangul glyphs are fixed-width/square and do not benefit from negative tracking — tight negative letter-spacing on Korean text tends to make characters collide/overlap. Negative tracking was **not** ported; headings use `0` or a small positive value instead.
- **Line-height**: bumped slightly higher than the Latin spec where body text is Korean, since Hangul composites (initial+medial+final jamo stacked in a block) need a touch more vertical room than Latin x-height text at the same size to stay readable.
- **Type scale**: Notion's 64/54/40px display sizes are marketing-hero sizes with nothing equivalent in this app (no hero section) — they were not imported. The smaller half of the scale (title/body-md/body-sm/caption/eyebrow) maps onto this app's headings/labels/body text, scaled down further in places to fit existing dense components (tables, kanban cards, calendar cells) rather than forcing marketing-site spacing into a utility UI.
- **Color tokens**: existing `--blue`/`--gray-*`/`--red`/`--green`/etc. CSS variables were repointed to Notion-equivalent values (primary blue `#0075de`, warm neutral ink/canvas scale, decorative accents) rather than renamed, so the change cascades through the whole stylesheet via the existing token references.
- **Danger red**: Notion's documented palette has no red (only pink/orange sit in "warm" territory) because a marketing site never needs a destructive-action color. This app has real delete/danger actions, so a warm brick-red was added as an extension, not pulled from the source palette.
- **Shapes**: primary/CTA buttons moved to pill (`rounded.full`); form inputs moved to Notion's tighter `rounded.xs` (not pill, per the source system's explicit rule); card radius aligned to `rounded.lg`/`rounded.md`.
